...
Documentation is a design deliverable because it does design work. It shapes decisions, prevents contradictions, and maintains fidelity over time. If nobody reads the CONVENTIONS.md, it's decoration. If every session reads it before making a change, it's governing the work. The document only matters when it's treated as a load-bearing structural element, not an artifact you filed and forgot.
