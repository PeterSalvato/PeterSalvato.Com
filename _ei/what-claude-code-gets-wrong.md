...
Each of these failures is predictable. Each one has a structural fix. The governance infrastructure exists because the model is consistent in ways that erode the specific qualities that make work belong to a person. The tool is excellent at processing. The failures are all in judgment, and the entire architecture is built around that boundary.
