...
Every texture on this site came through PressWorks. The grain on the page surface, the halftone treatments, the desaturated color palette. The [Formwork Protocol](/governance/formwork-protocol/) borrows the language directly: evaluation happens in distinct layers, like plates in a screen print. The metaphor is literal. The same plate-by-plate process, running in code instead of on a Heidelberg.

The tool is still being built. The operations work. The presets produce what I need. But the coverage isn't complete yet: img2img workflows, batch processing across an entire asset directory, tighter integration with the [Sovereign Design Engine](/infrastructure/sovereign-design-engine/) pipeline. I'm adding capabilities as the site demands them, still filling the cases.
